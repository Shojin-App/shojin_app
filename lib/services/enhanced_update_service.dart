...
class EnhancedUpdateService {
  ...
  void initializeProgressStream() {
    ...
    developer.log(
      'Progress stream initialized successfully',
      name: 'EnhancedUpdateService',
    );
  }
  ...
  void _updateProgress(UpdateProgress progress) {
    ...
    if (_progressController != null && !_progressController!.isClosed) {
      ...
      developer.log(
        'Warning: Progress controller is null or closed',
        name: 'EnhancedUpdateService',
      );
  ...
  bool isUpdateAvailable(String currentVersionStr, String latestVersionStr) {
    try {
      ...
      developer.log(
        'Error parsing version strings: $e',
        name: 'EnhancedUpdateService',
      );
      return false;
    }
  ...
  /// GitHubから最新リリース情報を取得
  Future<EnhancedAppUpdateInfo?> getLatestReleaseInfo(
    String owner,
    String repo, {
    bool silent = false,
  }) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonResponse['assets'] != null && jsonResponse['assets'] is List) {
          final assetDetails = _getPlatformSpecificAssetUrl(
            jsonResponse['assets'],
          );
        ...
        if (!silent) {
          developer.log(
            'GitHub Release: $cleanVersion, Asset: $foundAssetName',
            name: 'EnhancedUpdateService',
          );
        }
        ...
        if (!silent) {
          developer.log(
            'Failed to get latest release info: ${response.statusCode}',
            name: 'EnhancedUpdateService',
          );
        }
        return null;
      ...
      if (!silent) {
        developer.log(
          'Error fetching latest release info: $e',
          name: 'EnhancedUpdateService',
        );
      }
      return null;
  ...
  Future<String?> downloadUpdateWithProgress(
    EnhancedAppUpdateInfo releaseInfo,
  ) async {
    ...
    _updateProgress(UpdateProgress(progress: 0.0, status: 'ダウンロード準備中...'));
    ...
    try {
      ...
      await for (final chunk in response.stream) {
        ...
        if (_lastLoggedPercentage == -1 && currentPercentage == 0) {
          shouldLog = true;
        } else if (currentPercentage >= _lastLoggedPercentage + 10 &&
            currentPercentage < 100) {
          shouldLog = true;
        } else if (currentProgress >= 1.0 &&
            currentPercentage == 100 &&
            _lastLoggedPercentage < 100) {
          shouldLog = true;
        }

        if (shouldLog) {
          developer.log(
            'Progress Update: $currentPercentage%',
            name: 'EnhancedUpdateService',
          );
      ...
      if (!await file.exists()) {
        developer.log(
          'Error: File does not exist after download attempt (not due to cancellation). Path: ${file.path}',
          name: 'EnhancedUpdateService',
        );
        _updateProgress(
          UpdateProgress(
      ...
      if (totalBytes == 0 || downloadedBytes == totalBytes) {
        ...
        _updateProgress(
          UpdateProgress(
            ...
            totalBytes: totalBytes,
            errorMessage:
                'Downloaded size ($downloadedBytes) does not match total size ($totalBytes).',
            isCompleted: true,
          ),
        );
        if (await file.exists()) {
        ...
        _updateProgress(
          UpdateProgress(
            ...
            totalBytes: totalBytes,
            errorMessage:
                'Unexpected issue during download completion. Downloaded: $downloadedBytes, Total: $totalBytes',
            isCompleted: true,
          ),
        );
        if (await file.exists()) {
      ...
      if (filePath != null && await File(filePath).exists()) {
        try {
          await File(filePath).delete();
          developer.log(
            'Deleted incomplete file due to error: $filePath',
            name: 'EnhancedUpdateService',
          );
        } catch (deleteError) {
          developer.log(
            'Error deleting incomplete file: $deleteError',
            name: 'EnhancedUpdateService',
          );
        }
      }
  ...
  Future<void> markUpdateAttempt(String targetVersion) async {
    ...
    developer.log(
      'Marked update attempt for version: $targetVersion',
      name: 'EnhancedUpdateService',
    );
  }
  ...
  Future<bool> checkAndNotifyUpdateSuccess() async {
    try {
      ...
      if (pendingVersion != null && pendingVersion == currentVersion) {
        ...
        developer.log(
          'Update successful: $pendingVersion → $currentVersion',
          name: 'EnhancedUpdateService',
        );
        return true;
      }
      ...
      if (attemptTimestamp != null) {
        ...
        if (elapsed.inHours > 24) {
          ...
          await prefs.remove('update_attempt_timestamp');
          developer.log(
            'Cleared old pending update: $pendingVersion',
            name: 'EnhancedUpdateService',
          );
        }
      }
      ...
      developer.log(
        'Error checking update success: $e',
        name: 'EnhancedUpdateService',
      );
      return false;
    }
    ...
    try {
      ...
      if (version != null && timestamp != null) {
        return {
          ...
          'timeAgo': DateTime.now().difference(
      ...
      developer.log(
        'Error getting last update info: $e',
        name: 'EnhancedUpdateService',
      );
      return null;
    }
  ...
  Future<void> installUpdate(String apkPath) async {
    try {
      ...
      if (statusCode == null) {
        developer.log(
          'Installation failed: status code was null',
          name: 'EnhancedUpdateService',
        );
        return;
      }
      ...
      final installationStatus = AndroidPackageService.getInstallStatusByCode(
        statusCode,
      );
      developer.log(
        'Installation status: ${installationStatus.name}',
        name: 'EnhancedUpdateService',
      );

      if (installationStatus == InstallStatus.success) {
        developer.log(
          'Installation process started successfully',
          name: 'EnhancedUpdateService',
        );
      } else {
        developer.log(
          'Installation failed: ${installationStatus.name}',
          name: 'EnhancedUpdateService',
        );
      }
    } catch (e) {
  ...
  Future<void> applyUpdate(String apkPath, BuildContext context) async {
    ...
    try {
      ...
      _updateProgress(
        UpdateProgress(progress: 0.5, status: 'インストーラーを起動しています...'),
      );

      developer.log(
        'Attempting to install APK: $apkPath',
        name: 'EnhancedUpdateService',
      );
      final statusCode = await AndroidPackageService.installApk(apkPath);
      ...
      if (installationStatus == InstallStatus.success) {
        ...
        switch (installationStatus) {
          ...
          default:
            errorMessage =
                '予期せぬインストール状態です: ${installationStatus.name} (code: $statusCode)';
        }
        ...
        _updateProgress(
          UpdateProgress(
            ...
            isCompleted: true,
            errorMessage:
                'Installation failed with status: ${installationStatus.name} (code: $statusCode)',
          ),
        );
      }
    } catch (e) {
  ...
  Future<void> downloadAndInstallUpdate(
    ...
    try {
      ...
      developer.log(
        'Error in downloadAndInstallUpdate: $e',
        name: 'EnhancedUpdateService',
      );
      _updateProgress(
        UpdateProgress(
