...
全プラットフォームで、ユーザーに表示されるアプリ名を「Shojin」に統一しました。

- Android、iOS、macOS、Windows、Linux、Webのホーム画面・ウィンドウ・Webタイトルを「Shojin」に統一
- 日本語・英語のアプリタイトルと設定ファイルの共有文言を「Shojin」に統一
- パッケージ名、Bundle ID、アプリケーションID、実行ファイル名は変更せず、アップデート互換性を維持
