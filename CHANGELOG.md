...
- コードワークスペース中心の新しいエディター基盤へ刷新
- ホームウィジェットのカスタマイズとおすすめ問題表示を改善
- Material You配色、半透明ナビゲーション、タブアニメーションを含むUIの全面改良
- 狭い画面や拡大文字でも操作しやすいレスポンシブレイアウトに改善
- 認証が必要なAtCoderコンテストの取得とコンテスト通知の安定性を改善
- AtCoder問題文のMarkdown・TeX表示、入出力解析、元ページへの導線を改善
- ブラウザーのメタデータ取り扱いと送信導線の安全性を強化
- Flutter 3.44.6へ更新し、依存パッケージとAndroidビルド環境を更新
